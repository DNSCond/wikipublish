...
import { Datetime_global, Datetime_global_constructor } from "datetime_global/src/Datetime_global";
import { ClockTime, RelativeTime } from "datetime_global/src/RelativeTimeChecker";
...
import { template } from "./Revisions";
...
const style = Array.from(template.content.querySelectorAll('style.template-style'), style => style.outerHTML).join('');
  ...
  if (ff instanceof FakeFileFile && fileName) {
    ...
    file.setHeader('revision-Date', revisionDate); {
      ...
      if (wikipageName === 'favicond/personalprevent') {
        ...
        const innerText = jsonEncode(jsonContent, 2),
          pre = Object.assign(document.createElement('pre'), { innerText }),
        ...
        pre.append(jsonToHTML(jsonContent));
        pre.replaceWith(' Comming Soon!');
        details.style.borderTop = 'none';
      } file.append(html); {
        const { signal } = myAborter;
        Array.from(html.shadowRoot!.querySelectorAll('a'), a => {
...
export function attachAnchors(wikipageName: string, a: HTMLAnchorElement) {
